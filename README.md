...
> 本仓库公开已经完成并验证的前四问固件与算法。第五问的完整视觉闭环代码暂未纳入仓库，仅提供圆形自动闭环的实测演示视频。

## 项目实物与演示

![李萨如图形显示控制装置实拍](assets/device-prototype.jpg)

▶️ [第五问：圆形自动扫频、识别与闭环稳定演示视频](assets/question-5-circle-demo.mp4)

第五问使用摄像头观察示波器 X-Y 图形，通过粗扫、细扫和视觉反馈闭环寻找目标频率并稳定圆形。该视频用于展示整机联调结果，当前仓库中的可复用固件仍以前四问为主。

## 个人职责

本项目为三人组队完成。我担任**主要方案负责人和核心代码编写者**，主要负责系统总体方案、双ADC同步采样与信号处理、FFT与相位测量、数字PLL/FLL闭环、AD9833控制、电赛五问功能实现，以及软硬件联调和误差标定。
...
assets/
├─ device-prototype.jpg          装置实拍图
└─ question-5-circle-demo.mp4    第五问圆形闭环演示
```
